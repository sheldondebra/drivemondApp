...
trait CommonTrait
{
    public function calculateFinalFare($trip, $fare): array
    {
        ...
        if ($trip->type == 'parcel') {
            ...
            $parcel_payment = $trip->actual_fare - ($trip->coupon_amount ?? 0);
            ...
            return [
                'actual_fare' => round($trip->actual_fare, 2),
        ...
        if ($current_status === 'cancelled') {
            ...
            $actual_fare = $bid_on_fare ? $bid_on_fare->bid_fare : ($fare->base_fare + $distance_wise_fare_completed);
        } else {
            $actual_fare = 0;
        }
        ...
        $vat_percent = (double)get_cache('vat_percent') ?? 1;
        $final_fare_without_tax = ($actual_fare + $waiting_fee + $idle_fee + $cancellation_fee + $delay_fee)-($trip->coupon_amount ?? 0);
    ...
    public function estimatedFare($tripRequest, $routes, $zone_id, $tripFare = null, $area_id = null): mixed
    {
        if ($tripRequest['type'] == 'parcel') {
            ...
            $estimated_fare = [
                ...
                'estimated_fare' => round($est_fare, 2),
            ...
            $estimated_fare = $tripFare->map(function ($trip) use ($routes, $tripRequest, $area_id) {
                ...
                return [
                    ...
                    'estimated_fare' => $trip->vehicleCategory->type == 'car'
                        ? round(($trip->base_fare + $drive_fare), 2)
                        : round(($trip->base_fare + $bike_fare), 2),
